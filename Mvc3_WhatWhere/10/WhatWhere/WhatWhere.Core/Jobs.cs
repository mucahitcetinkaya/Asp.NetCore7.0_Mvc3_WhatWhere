...
using WhatWhere.Core.Models;
using WhatWhere.Entity.Concrete;

namespace WhatWhere.Core
{
    public static class Jobs
    {
        ...
        public static List<CustomCityType> ConvertCityType()
        {
            List<CityType> cities = GetCities();
            List<CustomCityType> newCities = new List<CustomCityType>();
            int counter = 1;
            foreach (CityType cityType in cities)
            {
                newCities.Add(new CustomCityType
                {
                    Id = counter,
                     Sehir = cityType.Il,
                      ParentId = null
                });
                int parentCityId = counter;
                counter++;
                foreach (var ilce in cityType.Ilceleri)
                {
                    newCities.Add(new CustomCityType
                    {
                        Id = counter,
                        Sehir = cityType.Il,
                        ParentId = parentCityId
                    });
                    counter++;
                }
            }
