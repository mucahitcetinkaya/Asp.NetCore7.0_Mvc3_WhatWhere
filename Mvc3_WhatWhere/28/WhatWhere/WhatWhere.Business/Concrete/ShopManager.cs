...
namespace WhatWhere.Business.Concrete
{
    public class ShopManager : IShopService
    {
        ...
        public async Task<List<Shop>> GetAllShopsWithCityTownCategoryAsync(bool isDeleted)
        {
            var result = await _shopRepository.GetAllShopsWithCityTownCategoryAsync(isDeleted);
            return result;
        }
